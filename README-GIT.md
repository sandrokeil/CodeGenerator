...
Your first step is to establish a public repository from which i can pull your work into the master repository.
 ...
 2. Fork the CodeGenerator repository (http://github.com/sandrokeil/CodeGenerator)
 3. Clone your fork locally and enter it (use your own GitHub username in the statement below)

    ```sh
    % git clone git@github.com:<username>/CodeGenerator.git
    % cd CodeGenerator
    ```

 4. Add a remote to the canonical CodeGenerator repository, so you can keep your fork
    up-to-date:

    ```sh
    % git remote add upstream https://github.com/sandrokeil/CodeGenerator.git
    % git fetch upstream
    ```

## Keeping Up-to-Date

Periodically, you should update your fork to match the canonical CodeGenerator repository. we have
added a remote to the CodeGenerator repository, which allows you to do the following:
...
When working on CodeGenerator, we recommend you do each new feature or bugfix in a new branch. This simplifies the
...
To ssh://git@github.com/sandrokeil/CodeGenerator.git
...
`https://github.com/sandrokeil/CodeGenerator/commits/<branch>.atom`

where &lt;branch&gt; is a branch in the repository.

To subscribe to git email notifications, simply watch or fork the CodeGenerator repository on GitHub.
